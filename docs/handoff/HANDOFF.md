...
| 2026-06-19 | 2 | #3 done | _pending_ | Rate limiting via `@nestjs/throttler`: global 60/min/IP guard + tighter `@Throttle` overrides (login 10, register 5, comment 20, reaction 30 per min); health `@SkipThrottle()`. New spec asserts login → 429. Backend suite **226/226** green; lint clean. |
