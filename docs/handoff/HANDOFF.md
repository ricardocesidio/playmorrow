...
| 2026-06-19 | 1 | #16 done | _pending_ | Dev-mode E2E path: `PLAYWRIGHT_DEV=1` serves with `next dev` (no production build) via `test:e2e:dev`; CI keeps `next start`. README documents both + the `NEXT_PUBLIC_*` inlining caveat. Verified public spec 8/8 in dev mode. |
