...
| 2026-06-19 | fix | CI: OAuth strategies missing env vars | current | Changed GoogleStrategy and GithubStrategy from getOrThrow to get with fallback so tests don't crash when OAuth env vars are absent. Cleaned unused imports in users.controller.ts. |
