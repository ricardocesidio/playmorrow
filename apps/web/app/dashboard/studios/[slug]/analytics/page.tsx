...
import {
  ArrowLeft, BarChart3, Eye, Heart, Target, MessageSquare, Users, Gamepad2,
  TrendingUp, TrendingDown, MonitorPlay, ArrowUpRight,
} from 'lucide-react';
import {
...
import { useStudio, useStudioAnalytics } from '@/lib/api/hooks';
...
export default function StudioAnalyticsPage() {
  ...
  const { data: analytics, isLoading, error } = useStudioAnalytics(slug);
      ...
      <main className="relative overflow-hidden bg-[#020609] px-3 pb-4 pt-3 text-foreground sm:px-5">
        ...
        <div className="relative mx-auto max-w-[1540px]">
          <div className="mb-6 flex items-center gap-3 p-3">
            <Link href={`/dashboard/studios/${slug}`} className="text-muted-foreground hover:text-cyan transition-colors">
              <ArrowLeft className="size-5" />
            </Link>
            <div>
              <h1 className="font-display text-2xl font-black uppercase tracking-tight text-white">
                {studioLoading ? 'Loading...' : studio?.name ?? 'Studio'} Analytics
              </h1>
              <p className="font-mono text-[0.55rem] text-muted-foreground">{slug}</p>
            </div>
          </div>

          {isLoading && !analytics ? <StudioAnalyticsSkeleton /> : error ? <StudioAnalyticsError /> : analytics ? (
            <>
              <StudioStatCards analytics={analytics} />
              <StudioChartSection analytics={analytics} />
...
function StudioStatCards({ analytics }: { analytics: { totalViews: number; uniqueVisitors: number; totalWishlists: number; totalFollowers: number; totalComments: number; viewsGrowth: number; wishlistsGrowth: number; followersGrowth: number; totalGames: number; gamesGrowth: number } }) {
  const cards = [
    ...
    { icon: <Gamepad2 className="size-5" />, label: 'Games', value: analytics.totalGames, growth: analytics.gamesGrowth, tone: 'cyan' as const },
  ];
...
function StatCard({ icon, label, value, growth, tone }: { icon: React.ReactNode; label: string; value: number; growth: number; tone: string }) {
...
function StudioChartSection({ analytics }: { analytics: { dailyViews: { date: string; count: number }[] } }) {
        ...
        {filteredData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={filteredData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="studioViewsGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <XAxis dataKey="date" tick={{ fill: '#6b7280', fontSize: 10, fontFamily: 'monospace' }} tickFormatter={(v: string) => { const d = new Date(v); return `${d.getMonth() + 1}/${d.getDate()}`; }} axisLine={{ stroke: 'rgba(62,231,255,0.15)' }} tickLine={false} />
              <YAxis tick={{ fill: '#6b7280', fontSize: 10, fontFamily: 'monospace' }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: '#050b0f', border: '1px solid rgba(62,231,255,0.2)', borderRadius: 0, fontSize: 12, fontFamily: 'monospace' }} labelStyle={{ color: '#3ee7ff' }} itemStyle={{ color: '#fff' }} />
              <Area type="monotone" dataKey="count" stroke={CHART_COLORS.cyan} fill="url(#studioViewsGradient)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" paddingAngle={2}>
                  {pieData.map((_, i) => <Cell key={i} fill={CHART_COLORS_ARRAY[i % CHART_COLORS_ARRAY.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ background: '#050b0f', border: '1px solid rgba(62,231,255,0.2)', borderRadius: 0, fontSize: 12, fontFamily: 'monospace' }} itemStyle={{ color: '#fff' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
...
function StudioAnalyticsSkeleton() {
  return (
