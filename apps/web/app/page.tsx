...
function LatestGameCard({ game }: { game: HomeGame }) {
  ...
  const barClass = game.status === 'ALPHA' ? 'bg-violet' : game.status === 'PRE_ALPHA' ? 'bg-amber' : 'bg-cyan';
  return (
      ...
      <div className="border-t border-border bg-background/50 p-4">
        {progress !== null && (
          <>
            ...
            <div className="h-1 bg-border"><div className={`h-full ${barClass} shadow-[0_0_12px_currentColor]`} style={{ width: `${progress}%` }} /></div>
          </>
        )}
...
type HomeGame = {
  ...
  tagline: string | null; progressPercent: number | null;
};
...
function normalizeLatestGames(games?: Game[]): HomeGame[] {
  ...
  return games.slice(0, 3).map((g) => ({
    ...
    tags: g.tags ?? [],
    tagline: g.tagline,
    progressPercent: g.progressPercent ?? null,
  }));
}
