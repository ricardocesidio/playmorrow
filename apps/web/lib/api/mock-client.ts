...
export function createMockApi() {
  const handler = {
    ...
    delete: <T>(path: string, _bodyOrToken?: unknown): Promise<T> => handleRequest('DELETE', path) as Promise<T>,
  };
...
async function handleRequest(method: string, path: string, _body?: unknown): Promise<unknown> {
  ...
  if (path.includes('/members')) {
    const sessionUser = mockSessionUser ?? MOCK_USER;
    return {
      members: [{
        id: 'member-1',
        ...
        user: { id: sessionUser.id, username: sessionUser.username, displayName: sessionUser.displayName, avatarUrl: null },
      }],
    };
  }
  ...
  // Wishlist
  if (path === '/me/wishlist') return { items: MOCK_GAMES.slice(0, 5).map((g, i) => ({ id: `wish-${i}`, game: g, createdAt: new Date().toISOString() })) };
